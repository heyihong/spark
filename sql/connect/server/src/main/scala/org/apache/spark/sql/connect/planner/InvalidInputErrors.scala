...
object InvalidInputErrors {
  ...
  def invalidSchemaDataType(dataType: DataType): InvalidPlanInput =
    invalidPlanInput("INVALID_SCHEMA_DATA_TYPE", Map("dataType" -> toSQLType(dataType)))
  ...
  def invalidStateSchemaDataTypeForFlatMapGroupsWithState(dataType: DataType): InvalidPlanInput =
    invalidPlanInput(
      "INVALID_STATE_SCHEMA_DATA_TYPE_FOR_FLAT_MAP_GROUPS_WITH_STATE",
  ...
  def invalidPythonUdtfReturnType(dataType: DataType): InvalidPlanInput =
    invalidPlanInput("INVALID_PYTHON_UDTF_RETURN_TYPE", Map("dataType" -> toSQLType(dataType)))

  def invalidOutputSchemaTypeForTransformWithStateInPandas(dataType: DataType): InvalidPlanInput =
    invalidPlanInput(
      "INVALID_OUTPUT_SCHEMA_TYPE_FOR_TRANSFORM_WITH_STATE_IN_PANDAS",
      Map("dataType" -> toSQLType(dataType)))
